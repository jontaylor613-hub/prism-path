  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${width} ${height}`} className={`w-full h-auto rounded-lg border bg-slate-900/50 border-slate-800`}>
        ...
        {data.map((d, i) => (<g key={i} className="group cursor-pointer"><circle cx={getX(i)} cy={getY(d.score)} r="4" fill="#0f172a" stroke={styles.chartLine} strokeWidth="2" className="transition-all duration-200 group-hover:r-6"/><rect x={getX(i) - 35} y={getY(d.score) - 45} width="70" height="30" rx="4" fill="#1e293b" stroke="#334155" className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none"/><text x={getX(i)} y={getY(d.score) - 26} textAnchor="middle" fontSize="10" fill="#e2e8f0" fontFamily="monospace" className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">{d.date}: {d.score}%</text></g>))}
      </svg>
    </div>
  );
};
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl p-8 relative z-10">
        ...
        <div className="space-y-4">
          <button onClick={() => onLogin({ name: "Pro Teacher", role, isPremium: true })} className="w-full py-3 bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-bold rounded-lg shadow-lg hover:shadow-fuchsia-500/20 transition-all">ENTER DASHBOARD</button>
        ...
        {activeTab === 'identify' && (
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card theme={'dark'} className="p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <textarea value={plaafp.data} onChange={e=>setPlaafp({...plaafp, data:e.target.value})} placeholder="Data Sources..." className={`w-full p-3 rounded outline-none h-24 ${styles.input}`} />
                            ...
                            <Button onClick={handleGeneratePlaafp} className="w-full mt-4" icon={Brain} theme={'dark'}>{isAnalyzing ? "Generating..." : "Generate Draft"}</Button>
                        </div>
                    </Card>
                    <Card theme={'dark'} className="p-6">
                        <h3 className={`text-lg font-bold mb-4 uppercase ${styles.text}`}>Output</h3>
                         {generatedPlaafp ? <div className="p-4 rounded border mt-4 whitespace-pre-wrap bg-slate-950 border-slate-800 text-slate-300">{generatedPlaafp}</div> : <div className="p-6 border border-dashed rounded flex flex-col items-center justify-center text-slate-500 h-64 border-slate-800"><Sparkles className="mb-2 opacity-50"/><p>Fill out the form to generate a draft</p></div>}
                    </Card>
                </div>
             )}
             ...
             {subTab === 'assessments' && (
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card theme={'dark'} className="p-6">
                        <h3 className={`text-lg font-bold mb-4 flex items-center gap-2 uppercase ${styles.text}`}><Upload size={20}/> Upload Data</h3>
                        <div className="space-y-4">
                            <input type="text" placeholder="Test Name" value={newAssessment.name} onChange={e => setNewAssessment({...newAssessment, name: e.target.value})} className={`w-full p-3 rounded outline-none ${styles.input}`} />
                            <textarea placeholder="Scores/Notes" value={newAssessment.desc} onChange={e => setNewAssessment({...newAssessment, desc: e.target.value})} className={`w-full p-3 rounded outline-none h-32 ${styles.input}`} />
                            <Button onClick={handleAddCustomAssessment} icon={Plus} theme={'dark'}>Add to Profile</Button>
                        </div>
                    </Card>
                    <Card theme={'dark'} className="p-6">
                        <h3 className={`text-lg font-bold mb-4 uppercase ${styles.text}`}>Library</h3>
                        <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                            {assessments.map((a, i) => (
                                <div key={i} className="p-3 rounded border bg-slate-950 border-slate-800 hover:border-fuchsia-500/50">
                                    <div className="flex justify-between items-start mb-1"><h4 className={`font-bold ${styles.text}`}>{a.name}</h4><Badge color="purple" theme={'dark'}>{a.type}</Badge></div>
             ...
             {subTab === 'impact' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card theme={'dark'} className="p-6">
                  <h3 className={`text-lg font-bold mb-4 uppercase ${styles.text}`}>Templates</h3>
                ...
                <Card theme={'dark'} className="p-6"><h3 className={`text-lg font-bold mb-4 uppercase ${styles.text}`}>Current Statement</h3><textarea className={`w-full p-4 rounded h-48 outline-none ${styles.input}`} value={plaafp.impact} onChange={(e) => setPlaafp({...plaafp, impact: e.target.value})} /></Card>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'develop' && (
          <div className="animate-fade-in space-y-6">
            ...
            {(subTab === 'wizard' || subTab === '') && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 <Card theme={'dark'} className="p-6">
                   <div className="flex justify-between items-center mb-6"><h3 className={`text-lg font-bold flex items-center gap-2 uppercase tracking-wide ${styles.text}`}><Target className="text-cyan-400" /> Goal Generator</h3><button onClick={() => handleAiAutoFill('goal')} className="text-xs flex items-center gap-1 px-3 py-1 rounded font-bold border transition-colors bg-fuchsia-900/20 text-fuchsia-400 border-fuchsia-500/50 hover:bg-fuchsia-900/40"><Wand2 size={12} /> ✨ Auto-Draft</button></div>
                   <div className="space-y-4">
                      <input type="text" className={`w-full p-2 rounded outline-none ${styles.input}`} placeholder="Condition" value={goalInputs.condition} onChange={e => setGoalInputs({...goalInputs, condition: e.target.value})} />
                      <input type="text" className={`w-full p-2 rounded outline-none ${styles.input}`} placeholder="Behavior" value={goalInputs.behavior} onChange={e => setGoalInputs({...goalInputs, behavior: e.target.value})} />
                      <input type="text" className={`w-full p-2 rounded outline-none ${styles.input}`} placeholder="Criteria" value={goalInputs.criteria} onChange={e => setGoalInputs({...goalInputs, criteria: e.target.value})} />
            ...
            {subTab === 'tracker' && (
               <div className="space-y-6">
                  <div className="flex justify-between items-center"><h2 className={`text-2xl font-black ${styles.text}`}>{trackers[0].name}</h2></div>
