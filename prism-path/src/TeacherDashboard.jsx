...
const getThemeClasses = (theme) => {
  // Forced Dark Mode for PrismPath Aesthetic
  return {
    bg: "bg-slate-950",
    text: "text-slate-200",
...
// THIS FIXES YOUR ERROR: Defining the constant exactly as used in state
...
const Badge = ({ children, color = "blue", theme = 'dark' }) => {
  ...
  return <span className={`px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 w-fit ${colors[color] || colors.blue}`}>{children}</span>;
};
...
const PaywallModal = ({ onClose, onUpgrade, onDistrictCode, theme = 'dark' }) => {
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center p-4 bg-slate-950/80">
      <div className="bg-slate-900 border-fuchsia-500/30 text-white border rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden relative flex flex-col md:flex-row h-[500px]">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-current transition-colors z-20">✕</button>
        <div className="w-full md:w-1/3 bg-slate-950 border-r border-slate-800 p-6 flex flex-col gap-2">
          <h3 className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-4">Select Plan Type</h3>
          <button onClick={() => setView('individual')} className={`p-4 rounded-xl border text-left transition-all ${view === 'individual' ? 'bg-fuchsia-900/20 border-fuchsia-500/50' : 'border-transparent'}`}><p className={`font-bold text-sm ${view === 'individual' ? 'text-white' : 'text-slate-500'}`}>Individual</p></button>
          <button onClick={() => setView('district')} className={`p-4 rounded-xl border text-left transition-all ${view === 'district' ? 'bg-cyan-900/20 border-cyan-500/50' : 'border-transparent'}`}><p className={`font-bold text-sm ${view === 'district' ? 'text-white' : 'text-slate-500'}`}>District</p></button>
        </div>
        <div className="flex-1 p-8 relative overflow-y-auto">
           ...
           {view === 'district' && (
              <div className="p-4 rounded-xl border mb-6 bg-slate-950 border-slate-800">
                <div className="flex gap-2"><input type="text" placeholder="ENTER CODE" className="flex-1 p-2 rounded text-sm outline-none border bg-slate-900 border-slate-700 text-white" value={districtCode} onChange={(e) => setDistrictCode(e.target.value)} /><button onClick={handleCodeSubmit} className="bg-cyan-600 text-white px-4 rounded text-xs font-bold">Unlock</button></div>
    ...
    <div className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center p-4 bg-slate-950/80">
      <div className="bg-slate-900 border-emerald-500/30 text-white border rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden relative flex flex-col max-h-[80vh] animate-fade-in-up">
        <div className="p-6 border-b flex justify-between items-start border-slate-800">
          <div><Badge color="green" theme={theme} className="mb-2">{strategy.category}</Badge><h2 className="text-2xl font-black uppercase tracking-wide">{strategy.title}</h2><p className="text-sm mt-1 text-emerald-400">UDL Focus: {strategy.udl}</p></div>
          <button onClick={onClose} className="p-2 hover:bg-black/10 rounded-full transition-colors"><X size={20} /></button>
        </div>
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
          <div className="space-y-2"><h4 className="text-xs font-bold uppercase tracking-widest opacity-70 flex items-center gap-2"><BookOpen size={14}/> Definition</h4><p className="text-sm leading-relaxed">{strategy.desc}</p></div>
          <div className="p-4 rounded-lg border bg-slate-950 border-slate-800"><h4 className="text-xs font-bold uppercase tracking-widest opacity-70 flex items-center gap-2 mb-3"><ListOrdered size={14}/> Implementation Steps</h4><div className="space-y-3">{strategy.steps && strategy.steps.map((step, idx) => (<div key={idx} className="text-sm flex gap-3"><span className="font-mono font-bold text-emerald-500">{(idx + 1).toString().padStart(2, '0')}</span><span>{step.replace(/^\d+\.\s*/, '')}</span></div>))}</div></div>
          <div className="grid grid-cols-2 gap-4"><div className="p-3 rounded border bg-amber-900/10 border-amber-500/20"><h5 className="text-xs font-bold uppercase mb-1 text-amber-400">Pro Tip</h5><p className="text-xs opacity-80">{strategy.tips}</p></div><div className="p-3 rounded border bg-blue-900/10 border-blue-500/20"><h5 className="text-xs font-bold uppercase mb-1 text-blue-400">Evidence Base</h5><p className="text-xs opacity-80">{strategy.evidence}</p></div></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/90 backdrop-blur-sm flex justify-center py-8">
      <div className="w-full max-w-4xl bg-slate-900 text-white rounded-xl shadow-2xl p-8 relative min-h-[80vh]">
        ...
        <div className="text-center border-b border-slate-200/20 pb-6 mb-8"><h1 className="text-3xl font-black uppercase tracking-tight mb-2">IEP Summary Draft</h1><p className="text-sm opacity-60 font-mono">CONFIDENTIAL: Generated by Prism Path | {new Date().toLocaleDateString()}</p></div>
        <div className="space-y-8">
          <section><h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-3 border-b border-slate-500/20 pb-1">I. Student Profile</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded bg-slate-500/5"><div><span className="font-bold block text-[10px] opacity-70 uppercase">Name</span> <span className="text-lg font-bold">{student.name}</span></div><div><span className="font-bold block text-[10px] opacity-70 uppercase">Grade</span> <span className="text-lg">{student.grade}</span></div><div><span className="font-bold block text-[10px] opacity-70 uppercase">Primary Need</span> <span className="text-lg">{student.need}</span></div><div><span className="font-bold block text-[10px] opacity-70 uppercase">Next IEP</span> <span className="text-lg font-mono">{student.nextIep}</span></div></div></section>
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-3 border-b border-slate-500/20 pb-1">II. Present Levels</h2><div className="p-4 rounded-lg border border-slate-800 h-full"><p className="leading-relaxed font-serif text-sm">{plaafp || "No PLAAFP generated."}</p></div></div><div><h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-3 border-b border-slate-500/20 pb-1">III. Parent Input</h2><div className="p-4 rounded-lg border border-slate-800 h-full"><p className="leading-relaxed font-serif text-sm">{parentInput || "No parent input."}</p></div></div></section>
          <section><h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-3 border-b border-slate-500/20 pb-1">IV. Goals</h2><div className="p-4 rounded-lg border border-slate-800"><div className="flex items-start gap-4"><Target className="mt-1 opacity-50" size={20}/><p className="leading-relaxed font-serif font-medium">{goal || "No goal generated."}</p></div></div></section>
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-3 border-b border-slate-500/20 pb-1">V. Services & LRE</h2><div className="p-4 rounded-lg border border-slate-800"><div className="mt-4 pt-4 border-t border-slate-500/20"><span className="block text-xs uppercase font-bold opacity-70 mb-1">LRE Percentage</span><span className="text-2xl font-black">{lre?.percentage ? `${lre.percentage}%` : "N/A"}</span></div></div></div></section>
        </div>
        <div className="mt-12 pt-6 border-t border-slate-200/20 flex justify-end gap-4 print:hidden"><Button onClick={onClose} variant="ghost" theme={theme}>Close</Button><Button onClick={() => window.print()} variant="primary" icon={Printer} theme={theme}>Print / Save PDF</Button></div>
      </div>
    </div>
  );
};
...
const LoginScreen = ({ onLogin, onBack }) => {
  ...
  const handleDevBypass = (isPremium) => { 
    ...
    onLogin({ name: "Pro Teacher", role, isPremium: isPremium, district, school, subject, grade }); 
  };
      ...
      <div className="max-w-md w-full bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl p-8 relative z-10">
        ...
        <div className="p-8 space-y-4">
          <div className="space-y-3">
              <div><label className="block text-xs font-bold text-cyan-400 uppercase mb-1">District</label><input type="text" className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-slate-200 focus:border-cyan-500 outline-none focus:ring-0" placeholder="e.g. Fayette County" value={district} onChange={e => setDistrict(e.target.value)} /></div>
              <div><label className="block text-xs font-bold text-cyan-400 uppercase mb-1">School</label><input type="text" className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-slate-200 focus:border-cyan-500 outline-none focus:ring-0" placeholder="e.g. Lincoln Elementary" value={school} onChange={e => setSchool(e.target.value)} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold text-fuchsia-400 uppercase mb-1">Subject</label><input type="text" className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-slate-200 focus:border-fuchsia-500 outline-none focus:ring-0" placeholder="e.g. Math" value={subject} onChange={e => setSubject(e.target.value)} /></div>
                <div><label className="block text-xs font-bold text-fuchsia-400 uppercase mb-1">Grade</label><input type="text" className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-slate-200 focus:border-fuchsia-500 outline-none focus:ring-0" placeholder="e.g. 3rd" value={grade} onChange={e => setGrade(e.target.value)} /></div>
              </div>
              <div>
                <label className="block text-xs font-bold text-emerald-400 uppercase mb-1">Role</label>
                <div className="relative"><select className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-slate-200 focus:border-emerald-500 outline-none appearance-none focus:ring-0" value={role} onChange={e => setRole(e.target.value)}><option className="bg-slate-900 text-slate-200">Special Education Teacher</option><option className="bg-slate-900 text-slate-200">General Education Teacher</option><option className="bg-slate-900 text-slate-200">Administrator</option></select><ChevronDown size={14} className="text-slate-500 pointer-events-none absolute right-3 top-3" /></div>
              </div>
          </div>
          <div className="pt-4 border-t border-slate-800"><p className="text-[10px] text-slate-500 text-center mb-3">DEV BYPASS</p><div className="grid grid-cols-2 gap-3"><button onClick={() => handleDevBypass(false)} className="text-[10px] text-slate-400 py-3 border border-slate-700 rounded hover:bg-slate-800 uppercase tracking-wider">Free Tier</button><button onClick={() => handleDevBypass(true)} className="text-[10px] text-amber-500 py-3 border border-amber-900/30 bg-amber-900/10 rounded hover:bg-amber-900/20 uppercase tracking-wider font-bold">Pro Tier</button></div></div>
        </div>
        <div className="px-8 pb-8"><button onClick={onBack} className="w-full py-2 text-slate-500 text-xs hover:text-white transition-colors">← Back to Main Site</button></div>
      </div>
    </div>
  );
};
...
export default function TeacherDashboard({ onBack }) {
  ...
  // Custom Data State (Linked to Assessment Library)
  ...
  const [serviceMinutes, setServiceMinutes] = useState({ totalSchoolMinutes: 1800, specialEdMinutes: 300 });
  ...
  const handleGeneratePlaafp = async () => {
    const prompt = `Role: Special Education Teacher. Task: Write a PLAAFP statement. Student: ${plaafp.name}, Strength: ${plaafp.strength}, Need: ${plaafp.need}, Data: ${plaafp.data}, Impact: ${plaafp.impact}. Write a concise paragraph.`;
    const result = await callAI(prompt);
    setGeneratedPlaafp(result);
  };

  const handleGenerateGoal = async () => {
    const prompt = `Role: SPED Teacher. Task: Write a SMART Goal. Student: ${activeStudent.name}, Need: ${activeStudent.need}, Condition: ${goalInputs.condition}, Behavior: ${goalInputs.behavior}, Criteria: ${goalInputs.criteria}, Measure: ${goalInputs.measurement}.`;
    const result = await callAI(prompt);
  ...
  const handleSelectImpact = (text) => { setPlaafp({ ...plaafp, impact: text }); showNotification("Impact Statement Updated"); };
